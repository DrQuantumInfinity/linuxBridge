...
typedef struct
{
   ...
   INT32 usartFileDescriptor;
   UINT8 rxBuf[SERIAL_RECEIVE_BUF_SIZE];
}USART;
/**************************************************************************
 *                                  Variables
 **************************************************************************/
 ...
 *                                  Prototypes
 **************************************************************************/
static void UsartConfigureSettings(USART *pUsart, char const *pDevice, USART_PARAMS const *pParams);
//callbacks
...
USART_HANDLE UsartRegister(char const *pDevice, USART_PARAMS const *pParams)
{
   ...
   if (usartIndex < NELEMENTS(usart))
   {
      ...
      if (pUsart->usartFileDescriptor >= 0 &&
          (pParams->mode == USART_MODE_RX || pParams->mode == USART_MODE_RX_TX))
      {
...
void UsartWriteBlocking(USART_HANDLE usartHandle, void const *pSrc, UINT32 length)
{
   ...
   USART *pUsart = UsartValidateHandle(usartHandle);
   INT32 writeResult = write(pUsart->usartFileDescriptor, pSrc, length);
...
static void UsartReceiveHandler(USART *pUsart)
{
   ...
   for (;;)
   {
      INT32 readLength = read(pUsart->usartFileDescriptor, pUsart->rxBuf, SERIAL_RECEIVE_BUF_SIZE);
      assert_param(readLength > 0);
      MsgPostDouble(MID_USART_RX, &dataReceivedEvent, sizeof(dataReceivedEvent), pUsart->rxBuf, (UINT32)readLength);
   }
}
static USART *UsartValidateHandle(USART_HANDLE usartHandle)
{
