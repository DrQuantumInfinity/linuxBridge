...
{
    log_info("New esp now device: %s", pPersist->name);
...
void TransportEspNow::Private::GoogleSendLightRgb(const ESP_NOW_DATA* pData, DeviceLightRGB* pDevice)
{
    ...
    */
    ...
    pDevice->SetLevel(static_cast<uint8_t>(pData->data.lightRgb.brightness * 100 / 255));
    pDevice->SetColourHS(static_cast<uint8_t>(pData->data.lightRgb.hue * 360 / 255), static_cast<uint8_t>(pData->data.lightRgb.saturation * 100 / 255));
}
