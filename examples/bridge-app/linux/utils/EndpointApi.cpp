...
{
    (void)chip::DeviceLayer::PlatformMgr().ScheduleWork(EndpointApiInitWorker);
    memset(&endpointApi, 0x00, sizeof(endpointApi));
}
void EndpointAdd(ENDPOINT_DATA *pData)
{
    (void)chip::DeviceLayer::PlatformMgr().ScheduleWork(EndpointAddWorker, reinterpret_cast<intptr_t>(pData));
}
void EndpointRemove(uint16_t deviceIndex)
{
    (void)chip::DeviceLayer::PlatformMgr().ScheduleWork(EndpointRemoveWorker, reinterpret_cast<intptr_t>((void*)(uint64_t)deviceIndex));
}
void EndpointReportChange(uint16_t deviceIndex, ClusterId cluster, AttributeId attribute)
{
    ...
    (void)DeviceLayer::PlatformMgr().ScheduleWork(EndpointReportUpdateWorker, reinterpret_cast<intptr_t>(path));
}
/**************************************************************************
...
// Actions cluster now uses delegate pattern - see bridged-actions-stub.cpp
/**************************************************************************
 *                                  Private Functions
 **************************************************************************/
static void EndpointApiInitWorker(intptr_t context)
{
    ...
    (void)emberAfSetDeviceTypeList(0, Span<const EmberAfDeviceType>(rootDeviceTypes));
    (void)emberAfSetDeviceTypeList(1, Span<const EmberAfDeviceType>(aggregateNodeDeviceTypes));
}
static void EndpointAddWorker(intptr_t context)
{
...
static void EndpointRemoveWorker(intptr_t context)
{
    ...
    if (endpointIndex < CHIP_DEVICE_CONFIG_DYNAMIC_ENDPOINT_COUNT)
    {
        ...
        (void)emberAfClearDynamicEndpoint(endpointIndex);
    }
    else
    {
