...
uint32_t AsciiHexToBytes(uint8_t* pDest, uint32_t destLen, char const* pSrc, char const** pSrcEnd)
{
   ...
   for (destCount = 0; (*pSrc != '\0') && (destCount < destLen); destCount++, pSrc += 2)
   {
      ...
      msb = static_cast<uint8_t>(tolower(pSrc[0]));
      msb = static_cast<uint8_t>(msb - (isdigit(msb) ? '0' : ('a' - 0xa)));
      lsb = static_cast<uint8_t>(tolower(pSrc[1]));
      lsb = static_cast<uint8_t>(lsb - (isdigit(lsb) ? '0' : ('a' - 0xa)));
      if ((msb > 0xf) || (lsb > 0xf))
      {
         break;
      }
...
{
   uint32_t i;
   uint32_t numWhole32BitWords = bufferSize & ~static_cast<uint32_t>(sizeof(uint32_t) - 1);
